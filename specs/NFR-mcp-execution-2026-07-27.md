...
| NFR-SEC-022 | Prompt-injection / Markdown-injection defense for LLM-facing or document-embedded untrusted text | `sanitize_untrusted_text` (control-character, line-separator, and bidi-override/isolate flattening to spaces, plus bidi-mark removal, plus length truncation) plus `wrap_untrusted_block` (`<untrusted-data>...</untrusted-data>`, `&`/`<`/`>`-escaped body) — applied by both `mcp-skill` and `mcp-server`, regression-tested against an explicit boundary-forgery attempt |
...
| NFR-SEC-025 | Visual-spoofing ("Trojan Source"-style) defense for untrusted text shown to a human or an LLM reader | Same mechanism as NFR-SEC-022 (`sanitize_untrusted_text`'s bidi-character flattening) — listed as its own requirement because it addresses a distinct threat (visual reordering/relabeling of already-well-formed text) rather than structural injection, even though one function implements both |
