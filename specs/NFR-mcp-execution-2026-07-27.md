...
| NFR-SEC-025 | Visual-spoofing ("Trojan Source"-style) defense, plus partial invisible-payload-smuggling defense against a specific enumerated character set, for untrusted text shown to a human or an LLM reader | Same mechanism as NFR-SEC-022 (`sanitize_untrusted_text`'s bidi-character flattening, plus its #425 handling of the Unicode Tags block and the enumerated zero-width characters — not variation selectors, a known gap) — listed as its own requirement because it addresses a distinct threat (text that misrepresents itself, or is entirely invisible, to the human reviewer even though an LLM reader sees it in full) rather than structural injection, even though one function implements both |
